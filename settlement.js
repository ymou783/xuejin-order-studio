...
function renderSettlement() {
  ...
  $("settlementRecord").textContent = `总撤离数 ${totalGames} 撤 · 成功撤离 ${successCount} 撤`;
...
function settlementSvg() {
  ...
  const record = `总撤离数 ${totalGames} 撤 · 成功撤离 ${successCount} 撤`;
  ...
  const row = (y, icon, label, value, className = "") => `<line x1="180" y1="${y - 25}" x2="820" y2="${y - 25}" stroke="#c9beb1" stroke-dasharray="5 6"/><text x="190" y="${y}" class="label"><tspan class="icon">${icon}</tspan> ${label}</text><text x="810" y="${y}" text-anchor="end" class="value ${className}">${escapeSvg(value)}</text>`;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1000" height="1600" viewBox="0 0 1000 1600">
    <defs><pattern id="settleGrain" width="12" height="12" patternUnits="userSpaceOnUse"><circle cx="2" cy="3" r=".7" fill="#6d5946" opacity=".08"/><circle cx="9" cy="8" r=".5" fill="#6d5946" opacity=".06"/></pattern><filter id="settleShadow" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="18" stdDeviation="18" flood-color="#57443c" flood-opacity=".22"/></filter><style>.ink{fill:#29232b;font-family:'Noto Sans SC','Microsoft YaHei',sans-serif}.label{fill:#29232b;font-size:34px;font-weight:800}.value{fill:#4d3c4c;font-size:31px;font-family:'DM Mono','Microsoft YaHei',monospace}.icon{font-size:35px}.small{fill:#806f79;font-size:22px;font-family:'Noto Sans SC','Microsoft YaHei',sans-serif}.red{fill:#bd5148}.green{fill:#6f9872;font-weight:700}.title{fill:#29232b;font-size:70px;font-weight:800;letter-spacing:8px}</style></defs>
    ...
    <text x="145" y="105" class="red" font-size="20">✦　⌁　✦</text><text x="855" y="105" text-anchor="end" class="ink" font-size="30" font-weight="800">雪烬电竞·</text><text x="500" y="254" text-anchor="middle" class="title">结单小票</text><rect x="346" y="276" width="308" height="42" rx="4" fill="#bd5148"/><text x="500" y="305" text-anchor="middle" fill="#fff9ef" font-size="23" font-weight="700" font-family="Noto Sans SC">✦ 三角洲小记 ✦</text>
    ...
    <line x1="180" y1="782" x2="820" y2="782" stroke="#c9beb1" stroke-dasharray="5 6"/><text x="190" y="820" class="label">▣ 订单号</text><text x="810" y="820" text-anchor="end" class="value" font-size="24">${escapeSvg(safeOrder)}</text><text x="190" y="858" class="small">备注：${escapeSvg(companionTrail)}</text>${row(900,"▥","战绩",record)}${row(950,"◉","撤离率",rate)}${row(1000,"¤","共得吃",eaten)}
    <path d="M145 1215 210 1165 254 1198 323 1115 378 1180 443 1130 510 1195 589 1100 654 1180 720 1135 784 1198 855 1148v67H145Z" fill="#2e2930"/><path d="M145 1217h710" stroke="#2e2930" stroke-width="4"/><g transform="translate(768 1168) rotate(-13)"><circle cx="0" cy="0" r="58" fill="#fffaf0" stroke="#bd5148" stroke-width="4"/><circle cx="0" cy="0" r="48" fill="none" stroke="#bd5148" stroke-width="2"/><circle cx="-10" cy="-8" r="5" fill="#bd5148"/><circle cx="0" cy="-13" r="5" fill="#bd5148"/><circle cx="10" cy="-8" r="5" fill="#bd5148"/><path d="M-11 5c0-9 22-9 22 0 0 9-22 9-22 0Z" fill="#bd5148"/></g>
    <text x="190" y="1260" class="label">▦ 日期</text><text x="810" y="1260" text-anchor="end" class="value">${date.long}</text><text x="500" y="1310" text-anchor="middle" class="red" font-size="26" font-weight="700">感谢板板支持</text><line x1="180" y1="1342" x2="820" y2="1342" stroke="#bd5148" stroke-opacity=".35"/><text x="190" y="1378" class="red" font-size="23" font-weight="700">本次服务评价</text><text x="190" y="1416" class="small">操作技术</text><text x="810" y="1416" text-anchor="end" class="small green">${escapeSvg(technical)}</text><text x="190" y="1454" class="small">服务态度</text><text x="810" y="1454" text-anchor="end" class="small green">${escapeSvg(attitude)}</text><text x="190" y="1492" class="small">老板备注</text><text x="810" y="1492" text-anchor="end" class="small">${escapeSvg(note)}</text><text x="500" y="1548" text-anchor="middle" class="red" font-size="22">感谢您的评价</text>
  </svg>`;
}

function saveSettlementImage() {
  if (saveInProgress || Date.now() < saveCooldownUntil) return;
  saveInProgress = true;
  ...
  image.onload = () => {
    ...
    canvas.toBlob((pngBlob) => {
      ...
      link.href = downloadUrl; link.download = `雪烬电竞-结单小票-${safeOrderId(orderId)}.png`; link.click();
